...
normal exporter does not consume it; `integration_preview.py` uses it only to build
the isolated branch review under `scripts/bzzoiro/review_export/`.

- `REPORT.md` is the human-readable result.
- `SUMMARY.json` contains structured pass/warn/fail checks.
- `RUN_MANIFEST.json` records the commit, inputs, runtime and schema hash.
